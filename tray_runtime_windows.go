...
func (t *TrayRuntime) createHiddenWindow() windows.HWND {
	className, _ := windows.UTF16PtrFromString("ClashMetaTrayWindow")
...
func (t *TrayRuntime) loadIcon() {
	...
	iconPath := filepath.Join(os.TempDir(), fmt.Sprintf("clashmeta_tray_%d.ico", os.Getpid()))
...
func (t *TrayRuntime) addTrayIcon() error {
	...
	if state.ActiveConfigName != "" {
		tip = "ClashMeta - " + state.ActiveConfigName
	}
...
func (t *TrayRuntime) renderOnTrayThread(state appcore.AppState) {
	...
	tooltip := "ClashMeta - " + txt.noConfig
	if state.ActiveConfigName != "" {
		tooltip = "ClashMeta - " + state.ActiveConfigName
	}
