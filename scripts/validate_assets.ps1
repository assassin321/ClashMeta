...
Write-Host "===== ClashMeta 构建资产校验 =====" -ForegroundColor Cyan
...
# 主程序
if (Assert-FileExists ".\build\bin\ClashMeta.exe" "主程序") {
    Assert-MZHeader ".\build\bin\ClashMeta.exe" "主程序" | Out-Null
}

# Helper
if (Assert-FileExists ".\build\bin\ClashMetaHelper.exe" "Helper 服务") {
    Assert-MZHeader ".\build\bin\ClashMetaHelper.exe" "Helper 服务" | Out-Null
}
