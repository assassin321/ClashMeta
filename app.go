...
func (a *App) startup(ctx context.Context) {
	...
	if !isSilent {
		...
		a.setWindowVisible(false)
		// 🚀 核心修复：消费 Windows STARTF_USESHOWWINDOW 机制，防止系统将首个 ShowWindow(SW_HIDE) 强制改写为显示
		sys.SafeHideMainWindowWin32()
		// 🚀 核心守护：在启动后 1 秒内确保窗口保持隐藏，避免 WebView2 初始化或系统消息导致闪烁露头
		sys.EnsureMainWindowHiddenSilently(func() bool {
...
func (a *App) InstallHelperService() error {
	exePath := filepath.Join(utils.GetAppDir(), "ClashMetaHelper.exe")
	if _, err := os.Stat(exePath); err != nil {
		return fmt.Errorf("ClashMetaHelper.exe 不存在于 %s，请先部署 helper 程序", exePath)
	}
...
func (a *App) SaveUwpExemptions(sids []string) error {
	if !sys.CheckAdmin() {
		// 普通权限模式下，借助 UAC 提权执行子进程以保存系统豁免配置
		tempFile := filepath.Join(os.TempDir(), fmt.Sprintf("clashmeta-uwp-%d.json", time.Now().UnixNano()))
		data, err := json.Marshal(sids)
		if err != nil {
			return fmt.Errorf("编码 UWP 列表失败: %w", err)
		}
		...
		if err := sys.RunElevatedWithArgsWait("--save-uwp-exemptions", tempFile); err != nil {
			return fmt.Errorf("提权保存 UWP 规则失败: %w", err)
		}
		return nil
	}
...
func (a *App) StopDelayTest() {
	a.core.Delay.StopDelayTest()
}
...
func (a *App) ExportBackup() (string, error) {
	savePath, err := runtime.SaveFileDialog(a.ctx, runtime.SaveDialogOptions{
		Title:           "选择备份保存位置",
		DefaultFilename: fmt.Sprintf("ClashMeta_Backup_%s.gocz", time.Now().Format("20060102")),
		Filters: []runtime.FileFilter{
			{DisplayName: "ClashMeta 备份文件 (*.gocz)", Pattern: "*.gocz"},
		},
	})
	if err != nil {
		return "", err
