; =========================================================
; ClashMeta - 智能适配安装脚本 (适配 paths.go 逻辑)
; =========================================================

#define MyAppName "ClashMeta"
#define MyAppVersion "1.3.0"
#define MyAppPublisher "assassin321"
#define MyAppExeName "ClashMeta.exe"

[Setup]
AppMutex=Global\ClashMeta_Single_Instance_Mutex
CloseApplications=yes
RestartApplications=no
WizardStyle=modern dynamic includetitlebar
VersionInfoVersion=1.3.0.0
VersionInfoCompany=assassin321
VersionInfoDescription=ClashMeta Installer
VersionInfoCopyright=Copyright (C) 2026 assassin321
; 基础信息
AppName={#MyAppName}
...
; 默认安装到 C:\Users\用户名\AppData\Local\Programs\ClashMeta
...
; --- 核心修改 2：降级权限要求 ---
; 软件安装和日常运行不需要管理员权限。
; TUN 模式通过 ClashMetaHelper 后台服务提供高权限能力，无需 UI 提权。
PrivilegesRequired=lowest

; 输出设置
OutputDir=.\build\installer
OutputBaseFilename=ClashMeta_win_amd64_Setup
...
Source: ".\build\bin\ClashMetaHelper.exe"; DestDir: "{app}"; Flags: ignoreversion
...
var
  DefaultDir: string;
begin
  DefaultDir := ExpandConstant('{localappdata}\Programs\ClashMeta');
...
begin
  if IsDefaultInstallDir() then
    Result := ExpandConstant('{localappdata}\ClashMeta')
  else
...
begin
  ...
  TestFile := AddBackslash(Dir) + '.clashmeta_write_test';
