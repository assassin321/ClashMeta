...
func CleanLegacyFiles(currentAppVersion string) {
	...
	updateTmp := filepath.Join(utils.GetDataDir(), "ClashMeta_update.exe.tmp")
	updateExe := filepath.Join(utils.GetDataDir(), "ClashMeta_update.exe")
	updateVer := filepath.Join(utils.GetDataDir(), "ClashMeta_update.version")
