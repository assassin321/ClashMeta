...
func RunElevatedWithArgsWait(args ...string) error {
	...
	os.Setenv("clashmeta_ADMIN_TASK_ID", taskID)
	// 提权子进程会继承此环境变量
...
func getAdminTaskResultPath() string {
	id := os.Getenv("clashmeta_ADMIN_TASK_ID")
	if id == "" {
		id = "default"
	}
	return filepath.Join(os.TempDir(), fmt.Sprintf("clashmeta_admin_task_%s.json", id))
}
