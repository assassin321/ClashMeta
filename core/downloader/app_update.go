...
func CheckAppUpdate(ctx context.Context, currentVersion string, strategy func() DownloadStrategy) (*AppUpdateInfo, error) {
	apiURL := "https://api.github.com/repos/assassin321/ClashMeta/releases/latest"
	...
	for _, client := range clients {
		...
		req.Header.Set("User-Agent", "ClashMeta-Updater")
...
func selectWindowsAsset(assets []struct {
	...
	for _, asset := range assets {
		...
		if !strings.Contains(lower, "clashmeta") {
			continue
		}
...
func DownloadAppUpdate(
	...
	if fileName == "" {
		// 兜底方案
		fileName = fmt.Sprintf("ClashMeta_%s_Setup.exe", strings.TrimPrefix(info.Version, "v"))
	}
