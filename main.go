...
func main() {
	...
	isDebugMode := false
	if filepath.Base(exePath) == "ClashMeta-dev.exe" || len(os.Getenv("WAILS_DEV_SERVER")) > 0 {
		isDebugMode = true
	...
	if hasFlag("--save-uwp-exemptions") {
		if !sys.CheckAdmin() {
		...
		for i, arg := range os.Args {
			if arg == "--save-uwp-exemptions" && i+1 < len(os.Args) {
		...
		if filePath == "" {
			err := fmt.Errorf("未指定 UWP 规则临时文件路径")
			logger.Errorf("%v", err)
			sys.WriteAdminTaskResult("save-uwp-exemptions", err)
			os.Exit(1)
		}

		data, err := os.ReadFile(filePath)
		if err != nil {
			logger.Errorf("读取 UWP 规则临时文件失败: %v", err)
			sys.WriteAdminTaskResult("save-uwp-exemptions", err)
			os.Exit(1)
		}
		...
		if err := json.Unmarshal(data, &sids); err != nil {
			logger.Errorf("解析 UWP 规则失败: %v", err)
			sys.WriteAdminTaskResult("save-uwp-exemptions", err)
			os.Exit(1)
		}

		if err := sys.SaveUwpExemptions(sids); err != nil {
			logger.Errorf("保存 UWP 规则失败: %v", err)
			sys.WriteAdminTaskResult("save-uwp-exemptions", err)
			os.Exit(1)
		}

		logger.Infof("成功保存 UWP 环回免除规则")
		sys.WriteAdminTaskResult("save-uwp-exemptions", nil)
		os.Exit(0)
	}
	...
	if hasFlag("--install-helper") {
		...
		helperPath := filepath.Join(filepath.Dir(exePath), "ClashMetaHelper.exe")
	...
	// 2. 单实例锁逻辑
	if !isDebugMode {
		mutexName, _ := syswin.UTF16PtrFromString("Global\\ClashMeta_Single_Instance_Mutex")
		...
		if err != nil {
			if err == syswin.ERROR_ALREADY_EXISTS {
				// 🚀 核心修复：若当前是由开机自启或静默模式拉起，遇到互斥锁碰撞说明已有实例在运行，
				// 此时绝不可调用 FocusMainWindowAndFlashTwiceWin32Only() 弹窗唤醒，必须直接静默退出！
				if hasFlag("--silent") || hasFlag("--startup") {
					logger.Infof("⚠️ ClashMeta 已有实例在运行，当前为静默/自启模式，直接退出，不唤醒已有窗口")
					if mutexHandle != 0 {
				...
				logger.Infof("⚠️ ClashMeta 已经在后台运行，正在唤醒已有窗口...")
		...
		// 🚀 核心自愈：清理可能残留的旧计划任务（防止开机时同时触发注册表Run与计划任务导致双启碰撞）
		go sys.DeleteLegacyTaskSchedulerTask()
...
func installEmergencyProxyCleanup() {
	...
	go func() {
		<-sigCh
		logger.Infof("检测到退出信号，正在清理 ClashMeta 设置的系统代理...")
		sys.ClearOwnedSystemProxy()
		os.Exit(0)
	}()
